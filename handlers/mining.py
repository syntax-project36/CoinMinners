...
""",
        ...
        # Retrieve wallet address from WALLETS dict
        wallet = WALLETS.get(coin, "Wallet address not configured.")

        # Retrieve miner info and calculate total amount
        miner_id = context.user_data.get("miner_id")
        quantity = context.user_data.get("quantity", 1)

        if miner_id and miner_id in MINERS:
            miner = MINERS[miner_id]
            total = miner["price"] * int(quantity)
        else:
            total = 0.0

        # Keyboard containing the 'I Have Paid' button and a 'Back' option
...
⚠️ *Send the exact amount to the address above. Once you have sent the transaction, tap the button below.*
""",
            parse_mode="Markdown",
